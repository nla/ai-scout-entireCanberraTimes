...
// 2 May - GTE results were disappointing, so now trying nomic
/*
 javac -cp . AddNomic768EmbeddingsToArticles.java
 java  -cp . AddNomic768EmbeddingsToArticles  ../newspapers/data/CTArticles1970OnwardsWithEntities.gz data/CT1970-WithNomic.gz ent
 5 hr elapsed
...
AddNomic768EmbeddingsToArticles ended articlesProcessed: 1785580
 
java  -cp . AddNomic768EmbeddingsToArticles  ../newspapers/data/CTArticlesTo1969.gz data/CT1926-169WithNomic.gz raw
...
AddNomic768EmbeddingsToArticles ended articlesProcessed: 1386475
 ...
 */
public class AddNomic768EmbeddingsToArticles {

	public static void main(String[] args) throws Exception {
	
		new AddNomic768EmbeddingsToArticles(new File(args[0]), new File(args[1]), args[2]) ;
	}
	
	int fatalErrors = 0 ;

	AddNomic768EmbeddingsToArticles(File in, File out, String runType) throws Exception {
			
		System.out.println("AddNomic768EmbeddingsToArticles in: " + in + ", out: " + out + " inputType: " + runType) ; 
    boolean raw = true ;
    if (runType.equals("ent")) raw = false ;
      ...
      br.close() ;
			...
			pr.flush() ;
			pr.close() ;
			System.out.println("AddNomic768EmbeddingsToArticles ended articlesProcessed: " + articlesProcessed) ;
    }
  ...
  float[] getEmbedding(String s) throws Exception {
    ...
    for (int maxLen = 4000;maxLen > 500; maxLen = maxLen - 200) {
      ...
      String req = "{\"model\":\"nomic-ai/nomic-embed-text-v1.5\"," + 
